...
public class Analista extends Empleado{
    ...
    // analista auxiliar, para que podamos obtener los datos expecificos sin introducir el resto del empleado
    public Analista(double sueldoExtra, int numProy){
        this(0);// esto da iwal por q no lo vamos a usar
        this.porcentajeExtra = sueldoExtra;
        this.numProyParticp = numProy;
    }
    
    //dia y mes iguales a los de hoy
    @Override
    public void checkCumpleMesAnio() throws CustomException{
        ...
        if((getFechaAlta()).get(DAY_OF_MONTH) == diaActual && (getFechaAlta()).get(MONTH) == mesActual){
            ...
            throw new CustomException("No cumple año\n");
        }
    }
